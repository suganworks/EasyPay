...
public class AuthServiceNUnitTests
{
    ...
    public void SetUp()
    {
        ...
        // App settings for ForgotPassword reset link
        var appSection = new Mock<IConfigurationSection>();
        appSection.Setup(s => s.Value).Returns("http://localhost:3000");
        _configMock.Setup(c => c["AppSettings:FrontendUrl"]).Returns("http://localhost:3000");

        // Email service always succeeds silently in tests
